...
export default function BottomNav() {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-gray-200 safe-area-bottom"
      role="navigation"
      aria-label="底部導航"
    >
        ...
        {tabs.map(tab => {
          ...
          return (
            <Link
              ...
              href={tab.href}
              className={cn(
                'relative flex flex-col items-center justify-center gap-0.5 min-w-0 flex-1 py-1 rounded-xl transition-all focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2',
                active
                  ? 'text-blue-600'
            ...
            >
              ...
              {active && (
                <span className="absolute bottom-1 w-1 h-1 rounded-full bg-blue-600" aria-hidden="true" />
              )}
            </Link>
          )
        })}
